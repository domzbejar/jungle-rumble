...
let noBalanceModal : Phaser.GameObjects.Container;
let noBet : Phaser.GameObjects.Image;
interface Bets{
...
export default class MainScene extends Phaser.Scene{
    ...
    preload(){
        ...
        // this.load.audio('chip-sfx','../audio/chip.mp3');
        // this.load.audio('click-sfx','../audio/click_01.mp3');
        // this.load.audio('chime-sfx','../audio/chime.mp3');
        // this.load.audio('jinggle-sfx','../audio/jinggle.mp3');
        // this.load.audio('error-sfx','../audio/error-tone-01.mp3');
        // this.load.audio('deselect-sfx','../audio/deselect.mp3');
        // this.load.audio('submit-sfx','../audio/blip.mp3');
        // this.load.audio('clear-chips-sfx','../audio/chips-reset.mp3');
        // this.load.audio('win-sfx','../audio/win_small.mp3');
        // this.load.audio('negative-tone-sfx','../audio/alert-negative-tone.mp3');

        //this.load.image('test','loading-page.jpg');
        this.load.bitmapFont('skarjan','skarjan.png','skarjan.xml');
        this.load.spritesheet('coin','coin.png',{frameWidth: 75, frameHeight : 75});
    }
    create(){

        //this.add.image(0,0,'test').setDepth(8);
        ...
        //const goldBtn = new BetButton(this,global.center.x,210,'btn-gold.png',"",0).setInteractive();
        ...
        /*
        const hawkSpine = this.add.spine(165,45,'hawk-spine','idle',true).setScale(.22).setDepth(9);
        const chickenSpine = this.add.spine(449,45,'chicken-spine','idle2',true).setScale(.42).setDepth(9);
        const pandaSpine = this.add.spine(825,45,'animal-spine','idle',true).setScale(.4).setSkinByName('panda').setDepth(9);
        const rabbitSpine = this.add.spine(636,47,'animal-spine','idle',true).setScale(.4).setSkinByName('rabbit').setDepth(9);
        const ostrichSpine = this.add.spine(353,50,'ostrich-spine','idle',true).setScale(.4).setDepth(9);
        const lionSpine = this.add.spine(915,47,'lion-spine','idle',true).setScale(.4).setDepth(9);
        */
        


        const animalButton : AnimalButton = {
            "hawk" : hawkBtn,
            //"golden" : goldBtn,
            "lion" : lionBtn,
            "chicken" :chickenBtn,
        ...
        global.chipPosition.x = chip1.x; //default 
        global.chipPosition.y = chip1.y; //default 
        ...
        chipButtonGroup.getChildren().forEach((item : Phaser.GameObjects.GameObject, i : number)=>{
            ...
            sprite.on('pointerdown',()=>{
                if( noBalanceModal.visible || sprite.data.values.isSelected )return
        ...
        highlightBox.getChildren().forEach((item,i)=>{
            ...
            const itemTween = this.tweens.add({
                ...
                onStart : (tween)=>{
                    //if(tween.progress == 0){
                        //this.sound.play('jinggle-sfx',{ volume : .3,rate : 3})
                    //}
                },
                onComplete : ()=>{
        ...
        const animalGroup = this.add.group({
            key : 'animals',
            // frame :['egg.png','owl.png','ostrich.png','chicken.png','egg.png','rabbit.png','monkey.png','panda.png','lion.png',
            // 'hawk.png','owl.png','ostrich.png','chicken.png',
            // 'egg.png','panda.png','monkey.png','rabbit.png','egg.png','chicken.png','ostrich.png','owl.png','hawk.png',
        ...
        submitBtn.on('pointerdown',()=>{
            if(global.onProcess || noBalanceModal.visible)return;
        ...
        // rebetBtn.on('pointerdown',()=>{
        //     if(global.onProcess || noBalanceModal.visible )return;
        //     this.sound.play('chip-sfx',{volume : .3 });
        //     reBet();
        // })

        const disableButtons = (isDisable : boolean = true)=>{
            if(isDisable){
        ...
        const RaysEffect = ( isActive : boolean = true, x : number = 0, y :number = 0) : void =>{
            if(isActive){
        ...
        const ShowWinAmount = (winAmount : number = 0, animal : string ='egg' )=>{
        ...
        const reBet = ()=>{
            ...
            for(let animal in betHistory){
                ...
                if(amount > 0){
                    ...
                    //animalButton[animal].AmountToChips(amount);
                }
            }
        ...
        const submitBets = (): string =>{
            ...
            if(order.totalAmount === 0){
                ...
                betHistory = bet;
                //globalBalance-=order.totalAmount
        ...
        const ClearAllChips = ()=>{
            betButtonGroup.forEach( (btn : BetButton) =>{
                btn.ClearBets();
            })
            BetOrder.TotalBetAmount = 0;
        ...
        const readyProcess = ()=>{
            // animalGroup.getChildren().forEach((item,i)=>{
            //     const animal = item as Phaser.GameObjects.Sprite
        ...
        //console.log( this.highlightBoxTweenGroup[0] )
    }
    update(time: number, delta: number): void {
        //console.log( this.highlightBoxTweenGroup[0].progress )
    }
}
...
class BetButton extends Phaser.GameObjects.Sprite{
    ...
    scene: Phaser.Scene;
    totalBet : number = 0;
    totalBetDisplay !: Phaser.GameObjects.Text;
    maxBet : number = 1000;
    chipSpriteGroup !: Phaser.GameObjects.Group;
    buttonSprite !: Phaser.GameObjects.Sprite;
    particleEmitter !: Phaser.GameObjects.Particles.ParticleEmitter;
    flicker !: Phaser.Tweens.Tween;
    fxPost !: Phaser.FX.Bloom;
    animalType : string;
    maxText : Phaser.GameObjects.Text;
    winAmoutDisplay !: Phaser.GameObjects.Container;
    constructor(scene : Phaser.Scene,x : number,y:number,texture:string,animalType:string, multiplier : number ){
        ...
        //console.log( this.buttonSprite.getBounds() )
        this.chipSpriteGroup = scene.add.group();

        const fx = this.buttonSprite.preFX?.addBloom(0xffffff,0,0,1.3,1).setActive(false);
        this.fxPost = this.buttonSprite.postFX?.addBloom(0xffffff,0,0,1,1);
        ...
        //const winAmountBg = this.scene.add.image(this.x,this.y-20,'main','win-amount-bg.png');
        ...
        this.buttonSprite.on('pointerdown',()=>{
            ...
            //console.log("domz lng ang igop");
            this.AddBet()
        }).on('pointerover',()=>{
            if(global.onProcess)return
            fx?.setActive(true);
        }).on('pointerout',()=>{
            fx?.setActive(false);
        })
        ...
        // this.maxText = this.scene.add.text(this.x + (this.width/2) - 25,this.y - (this.height/2) + 15,'最大',{
        //     color :"#cb3329",
        //     fontSize : 18,
        //     fontStyle : 'bolder',
    ...
    AddBet(amount : number = 0){
       // console.log("ADD BET")
        if(this.totalBet >= this.maxBet)return this.scene.sound.play('all-sfx',sfxMarker[0]);
    ...
    ChipEffect( chipDenom ?: number ){
        ...
        this.chipSpriteGroup.addMultiple([ chip as Phaser.GameObjects.Sprite ]);
    ...
    WinEffect(isOn:boolean = true, withParticle : boolean = false){
        if(isOn){
            if(withParticle){
    ...
    AmountToChips(amount : number){
        const chipDenom  : number[] = AmountToDenom(amount);
...
function AmountToDenom( amount : number):number[]{
    let total = amount;
    const result : number[] = [];
...
class HistoryPanel extends Phaser.GameObjects.GameObject{
    x : number;
    y : number;
    animalGroup : Phaser.GameObjects.Group;
    colorGroup : Phaser.GameObjects.Group;
    animalHistory : string[] = [];
    ...
    AddHistory(animal: string){
        ...
        const maxHistoryCount : number = 23;
        //this.animalGroup
        ...
        //console.log( this.animalHistory )
    }
}